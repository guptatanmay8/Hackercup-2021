...
void solve()
{
    ...
    unordered_map<int,int> m_old;
    unordered_map<int,int> m_underneath_old;
    for(int i=0;i<m;i++)
    {
        cin>>x;
        m_old[x]++;
    }
    m_underneath_old = m_old;
    ...
    fr(i,0,n)
    {
        unordered_map<int,int> m_new, m_underneath_new;
        ...
        fr(j,0,m)
        {
            ...
            else
            {
                ...
                if(m_underneath_old[x]>0 && m_old[x]<= m_underneath_old[x])
                {
                    m_underneath_new[x]++;
                    m_underneath_old[x]--;
                }
        ...
        m_old = m_new;
        int c2 =0;
        for(auto x: m_underneath_new)
        {
            c2+= x.second;
        }
        m_underneath_old  = m_underneath_new;
        c = max(0, c - (c1-c2));
        c1 = c2;
        // cout<<c<<" ";
    }
